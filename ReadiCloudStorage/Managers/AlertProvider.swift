...
class AlertProvider {
    ...
    // Simple Alert with textfield and action
    func showAlertWithTextFieldAndAction(title: String?, message: String, action: AlertAction, completion: @escaping (_ action: UIAlertAction, _ input: String) -> ()) {
        
        let alertController = UIAlertController(title: title, message: message, preferredStyle: .alert)
        
        alertController.addTextField { _textField in
            _textField.placeholder = "Name"
            _textField.keyboardType = .default
            _textField.textContentType = .name
            _textField.autocapitalizationType = .words
        }
        
        let _action = UIAlertAction(title: action.title, style: action.style) { (action: UIAlertAction) in
            completion(action, alertController.textFields?.first?.text ?? "")
        }
